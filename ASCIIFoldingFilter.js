/* Adapted from org.apache.lucene.analysis.miscellaneous.ASCIIFoldingFilter v 4.10.1 */
 ...
 * @return ouput and length of output
 */
ASCIIFoldingFilter.prototype.foldToASCII=function(input, options) {
  var optionsDefault={
    inputPos: 0,
    output: "",
    outputPos: 0,
  ...
  options=options||{};
  for(var o in optionsDefault) {
    if(!options.hasOwnProperty(o)) {
      options[o]=optionsDefault[o];
    }
  }

  var end = options.inputPos + options.length,
    output=[];
  for (var pos = options.inputPos; pos < end ; ++pos) {
    var c = input[pos];

    // Quick test: if it's not in range then just keep current character
    if (c < '\u0080') {
      output[options.outputPos++] = c;
    } else {
      switch (c) {
        ...
        case '\uFB03': // ﬃ  [LATIN SMALL LIGATURE FFI]
          output[options.outputPos++] = 'f';
          output[options.outputPos++] = 'f';
          output[options.outputPos++] = 'i';
          break;
        case '\uFB04': // ﬄ  [LATIN SMALL LIGATURE FFL]
          output[options.outputPos++] = 'f';
          output[options.outputPos++] = 'f';
          output[options.outputPos++] = 'l';
          break;
  ...
  options.output=output.join('');
  return {output:options.output, position:options.outputPos};
}

var aff = new ASCIIFoldingFilter();

function ASCIIFoldingFilterStream(options) {
  if(!(this instanceof ASCIIFoldingFilterStream)) {
    return new ASCIIFoldingFilterStream(options);
  }

  stream.Transform.call(this, options);

  if(options && options.encoding!=='utf8') {
    throw new Error('Filter stream valid only for UTF-8 text streams');
  }
}

util.inherits(ASCIIFoldingFilterStream, stream.Transform);

ASCIIFoldingFilterStream.prototype._transform=function(chunk, encoding, callback) {
  var i, src, trg;

  for(i=0; i<chunk.length; i++) {
    src=chunk[i];
    trg=aff.foldToASCII(src);
    this.push(trg.output);
  }
  callback();
};

module.exports= { ASCIIFoldingFilter: aff, ASCIIFoldingFilterStream: ASCIIFoldingFilterStream};
